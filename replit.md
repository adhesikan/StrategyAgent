...
### VCP Trader AI Pricing & Data-Mode Refresh (May 2026)
Public-facing brand is **VCP Trader AI** (Strategy Agent retained as engine sub-brand). Pricing is one plan — **VCP Trader AI Pro $99/month with a 14-day free trial** (no Pro/Elite split publicly). The Stripe checkout endpoint already passes `trial_period_days: 14`; the existing `pro` planId is reused so the user only needs to update the Stripe price behind `STRIPE_PRO_MONTHLY_PRICE_ID` to a $99 recurring price. Three standardized data modes are surfaced via `BrokerStatusStrip`: **Live Broker Mode** (connected non-sandbox), **Paper Mode** (connected sandbox account, e.g. `sandbox:` prefix), **Simulated Examples** (no broker — fallback for learning). InstaTrade™ ticket button label adapts: `Connect Broker to Use InstaTrade™` (disabled, no account) → `Paper Trade` (sandbox account) → `Send to Broker with InstaTrade™` (live account). TradeGuard™ language replaced with "Risk Controls / Built-In Risk Checks". Compliance footer (`client/src/components/footer.tsx`) and `ComplianceFooter` in `trading-shell.tsx` rewritten per spec §12. FAQ rewritten on landing page with the 8 Q&As covering trial, paper-data caveats, automation, advice disclaimer, and broker support.
