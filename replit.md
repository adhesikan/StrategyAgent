...
## Internal Market-Data API (service-to-service)
`GET /api/internal/market/history?symbol=MU&interval=1day&outputSize=120` (`server/routes/internal-market.ts`) — for the external vcp-trader-mcp service. Reuses the production Twelve Data daily provider (`TwelveDataDailyProvider.getDailyBars`); no new market-data logic. Returns `{symbol, interval, candles:[{timestamp,open,high,low,close,volume}]}` ordered oldest→newest. Only `interval=1day`; `outputSize` 1–500 (default 120). Auth: `Authorization: Bearer <VCP_INTERNAL_API_KEY>` (secret; constant-time compare, never logged, never sent to browser; no user/session auth). 401 missing/invalid token, 503 `INTERNAL_API_DISABLED` if the secret is unset. Structured errors `{error:{code,message}}`; 20s route timeout on top of provider timeout/retries.

## CongressFlow Widget (external iframe)
External congress-trading widget at `https://congress.vcptrader.com/embed`, integrated iframe-only (no backend). `client/src/lib/congressflow.ts` (trusted origin constant, safe URL builder, ticker/slug/ISO-date validation; tests `client/src/lib/congressflow.test.ts`) + `client/src/components/congressflow-embed.tsx` (postMessage origin+source validation, auto-height clamp, READY-gated command queue for SET_TICKER/SET_DATE_RANGE/SET_WATCHLIST, loading/slow/error/retry, analytics `congressflow_*` events). Surfaces: `/markets/congress-activity` (+`/politician/:slug`) page, Market Intel link card, Opportunity Radar per-ticker drawer, trade-detail collapsible section. All surfaces carry disclosure copy (delayed/amended disclosures, not a signal). Customer-facing "Congress Activity" branding is allowed here (July 2026) — supersedes the earlier removal note for Market Intel lists.
