...
import { registerOpportunityChangesExplainedRoute } from "./routes/opportunity-changes-explained";
...
import { registerResearchReportRoutes } from "./routes/research-reports";
...
import { registerOrderPreparationRoutes, ensureOrderDraftTables } from "./routes/order-preparation";
  ...
  app.use((req, res, next) => {
    ...
    if (accept.includes("application/json")) {
      return res.status(503).json({
        ...
        message: "VCP Trader AI is undergoing scheduled maintenance. Please check back shortly.",
        maintenance: true,
      });
    }
...
<title>VCP Trader AI - Maintenance</title>
<style>
...
<p style="font-size:.875rem">If you need immediate help, contact <a href="mailto:support@sunfishtrading.com" style="color:#60a5fa">support@sunfishtrading.com</a></p>
  ...
  // Must be before /api/opportunities/changes (exact) to avoid route collision
  registerOpportunityChangesExplainedRoute(app, isAuthenticated);
  ...
  // Signal routes (/api/institutional/signals/*) must come before the dynamic
  ...
  registerOperationsManualRoutes(app, isAuthenticated, isAdmin);
  registerPortfolioRoutes(app, isAuthenticated);
  ...
  registerOrderPreparationRoutes(app, isAuthenticated);
  ensureOrderDraftTables().catch((e: any) => console.error("[order-prep] startup table init failed:", e?.message));
  registerTradePreferencesRoutes(app, isAuthenticated);
  ensureTradePreferencesTables().catch((e: any) => console.error("[trade-preferences] startup table init failed:", e?.message));
  registerEquityPreviewRoutes(app, isAuthenticated);
  ...
  registerTestLiveCertificationRoutes(app, isAuthenticated, isAdmin);
  ...
  ensureResearchReportsTables().catch(err =>
    console.error("[research-reports] startup table init failed:", err?.message)
  );
  ensurePortfolioHistoryTables().catch(err =>
    console.error("[portfolio-history] startup table init failed:", err?.message)
  );
  ...
  app.post("/api/algo-pilotx/test", isAuthenticated, async (req, res) => {
    try {
      ...
      const testPayload = {
        type: "test",
        timestamp: new Date().toISOString(),
        message: "VCP Trader AI connection test",
      };
