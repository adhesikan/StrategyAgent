...
- Confidence must reflect data completeness and analytical agreement only — never bullishness/bearishness. A clearly bearish structural verdict with complete data is still high confidence.`;
    ...
    if (opts.rankedTradeSearch) {
      ...
      // Explanation only.
      mcpTools = [];
      mcpSystemRules += `\n- The "rankedTradeSearch" field in the user content is the DETERMINISTIC output of the production market-ranking engine. Your job is ONLY to explain it. You may NOT reorder candidates, promote a watch or rejected result to a trade, invent candidates, invent triggers/targets/risk/quantity, call rejected or watch-only setups trades or trade ideas, change the requested count, or override risk failures. "reviewedCount" is the number of RAW STORED OPPORTUNITIES reviewed; the qualified/watch/rejected/unavailable buckets count post-confluence candidates and may NOT sum to reviewedCount — never present them as the same population. Only entries in "candidates" are trade candidates. State counts exactly as given. If a maximum-risk budget was requested and no candidate survived, AND the backend rejectionSummary confirms that risk-limit failure was the primary terminal reason (RISK_LIMIT_EXCEEDED groups dominate and excluded candidates are fewer than rejected candidates), then say explicitly that no candidate met that risk limit. CRITICAL: Do NOT claim risk-limit failure if most candidates were excluded BEFORE qualification — excluded candidates may never have reached risk evaluation at all. If exclusions dominate (excludedCount > rejectedCount), use language such as "no candidate currently qualifies for the request" rather than claiming every candidate failed the risk test. CRITICAL EXCLUSION RULE: "exclusionSummary" entries represent opportunities that were filtered out BEFORE confluence and qualification — they are NOT quality rejections or risk failures. Do not describe excluded opportunities as rejected, failed on quality grounds, low quality, or having poor risk/reward. The "excludedCount" reflects pre-qualification filtering (e.g., no active trigger), not post-qualification judgment. Explain an exclusion as a data or timing issue, not a quality verdict. UNAVAILABLE RULE: "unavailableCount" and any rejection groups with data-availability reasons (DATA_UNAVAILABLE, UNDERLYING_MARKET_DATA_UNAVAILABLE, OPTIONS_DATA_UNAVAILABLE, DATA_FRESHNESS_INSUFFICIENT, MARKET_REGIME_UNAVAILABLE) indicate market data was missing — these are NOT quality rejections or qualification failures. Never describe unavailable setups as rejected, low quality, or failing risk/reward. CLOSEST MATCH RULE: Never invent closest-match or near-miss candidates. If no candidate qualified, say so plainly — do NOT suggest setups that "almost" qualified unless the deterministic result explicitly contains them. PORTFOLIO RULE: If buyingPower shows as sufficient and cashSufficiency shows as not_verified, do NOT combine these into a statement that the trade is affordable — they are separate signals. Affordability is NOT confirmed unless both are verified. COUNT INTEGRITY: Do not recalculate, change, or restate any count differently from the deterministic result. PRESENTATION RULE: The user already sees a deterministic engine summary with exact counts. Do NOT repeat reviewedCount, qualifiedCount, excludedCount, unavailableCount, or any other specific numbers from the result — the user can already see them in the structured card. Instead focus on: (1) what happened and why in plain language, (2) what the specific constraint or issue was, (3) what the user should do next. Prefer qualitative explanation over quantitative repetition. Example: "The deterministic engine rejected most opportunities before qualification because they lacked sufficient evidence to become actionable trades. Market data was unavailable for a few additional candidates. Returning no trade is an intentional safety feature." Do not call any tools for this request.`;
    }
  ...
  app.post("/api/ask", isAuthenticated, async (req: any, res) => {
    try {
      ...
      res.json({
        ...
        suggestions,
        source,
        disclaimer: "This analysis summarizes deterministic technical research generated from market data and predefined qualification rules. It is intended to support your own investment research and is not personalized investment advice.",
