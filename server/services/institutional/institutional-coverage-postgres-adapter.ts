        ...
        const adapter: CoverageApplyTransaction = {
          ...
          async updateHoldings(operation) {
            const result = await executor.execute(sql`
              ...
              WHERE f.accession_number=h.accession_number AND f.is_effective=TRUE
                AND h.cusip=${operation.cusip} AND h.put_call IS NULL
                AND h.shares_prn_type IS DISTINCT FROM 'PRN' AND h.reported_shares>0
